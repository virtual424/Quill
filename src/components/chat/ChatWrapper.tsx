...
export default function ChatWrapper({ fileId, isSubscribed }: ChatWrapperProps) {
  const { data, isPending } = trpc.getFileUploadStatus.useQuery(
    ...
    {
      refetchInterval: ({ state: { data } }) => (data?.status === "SUCCESS" || data?.status === "FAILED" ? false : 500),
    }
  );
        ...
        <div className="flex-1 flex justify-center items-center flex-col mb-28">
          <div className="flex flex-col items-center gap-2">
            ...
            <p className="text-zinc-500 text-sm">
              ...
              {isSubscribed ? PLANS.find((p) => p.name === "Pro")?.pagesPerPdf : PLANS.find((p) => p.name === "Free")?.pagesPerPdf} pages
              per PDF.
            </p>
            <Link
              href="/dashboard"
